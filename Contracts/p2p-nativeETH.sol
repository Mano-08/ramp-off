...
contract EXGETstarted is FunctionsClient, ConfirmedOwner {
   ...
   error UnexpectedRequestID(bytes32 requestId);
    ...
    event EthClaimed(address indexed seller,address indexed buyer, uint256 tradeId, uint256 amountClaimed);
...
modifier onlyParticipant(uint256 tradeId) {
    require(
        (trades[tradeId].buyer.addr == msg.sender || trades[tradeId].seller.addr == msg.sender) &&
...
modifier OnlySeller(uint256 tradeId) {
    ...
    function startRound  (
        ...
        require(msg.value == trades[tradeId].EthLock, "Incorrect ETH amount sent by seller");
...
function submitProposalOffRAMP(uint256 tradeId,uint256 tradeETH ) external {
    ...
    function confirmOffRamp(address sellerAddress,uint256 tradeId, bytes memory _data) external returns (uint64) {
        ...
        if (trades[tradeId].seller.addr==sellerAddress) {
            ...
            recipients[1] = abi.encode(partyB);
             bytes memory data = abi.encode(sellerAddress, partyB,trades[tradeId].EthLock, _data);
          ...
          emit attestationEvent(attestationId);
           trades[tradeId].agreed=true;
          trades[tradeId].buyer = Merchant(msg.sender, trades[tradeId].EthLock);
    ...
    function claim(uint256 tradeId)  external payable OnlySeller(tradeId){
        ...
        require(trades[tradeId].INR_Paid_to_Seller > trades[tradeId].InrAmount*83, "you didnt transfer enough funds to your seller" );
        trade.EthLock = 0;
        (bool success, ) = payable(trades[tradeId].buyer.addr).call{value: amount}("");
        require(success, "Failed to send Ether to seller");
        emit EthClaimed(trades[tradeId].buyer.addr,trades[tradeId].seller.addr, tradeId, amount);
    }
